...
ui <- fluidPage(
  titlePanel("SEIRS (Counts) — Waning Immunity, No Vital Dynamics"),
  sidebarLayout(
    sidebarPanel(
      numericInput("N", "Population size (N):", value = 1e6, min = 1, step = 1),
      sliderInput("beta",  "Transmission rate β (/day):", min = 0, max = 2, value = 0.35, step = 0.01),
      ...
      sliderInput("omega", "Waning rate ω (/day):",       min = 0, max = 1, value = 0.01, step = 0.001),
