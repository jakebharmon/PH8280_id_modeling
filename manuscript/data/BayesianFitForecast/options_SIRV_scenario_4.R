#Scenario 4 – Coexistence under weak cross-immunity
#When cross-immunity is weak both strains are able to circulate concurrently. 
#Each produces a substantial epidemic wave, and their peaks overlap in time. 
#This demonstrates that the system can maintain coexistence when the antigenic 
#distance between strains is large, as predicted by the analytical conditions.
...
model_name <- "scenario_4_coexistence_weak_crossimm"
...
params11_prior <- "normal(0.5, 0.05)T[0,]"  # sigma12: partial immunity from strain 1 → strain 2
params11_LB <- 0
params11_UB <- 1
params12_prior <- "normal(0.5, 0.05)T[0,]"  # sigma21: partial immunity from strain 2 → strain 1
params12_LB <- 0
params12_UB <- 1
