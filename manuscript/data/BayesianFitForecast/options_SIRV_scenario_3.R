#Scenario 3 – Competitive exclusion (strong cross-immunity)
#In this test, strong cross-immunity is introduced so that infection with one 
#strain nearly eliminates susceptibility to the other. Strain 1, which has the 
#slightly higher reproduction dominates and suppresses the second strain 
#entirely. The result reproduces the competitive-exclusion principle described 
#in the text, confirming that cross-protection terms are functioning properly.
...
model_name <- "scenario_3_competitive_exclusion"
...
params2_LB <- 0
params2_UB <- 10
params3_prior <- "normal(3.0, 0.7)T[0,]"
params3_LB <- 0
params3_UB <- 10
