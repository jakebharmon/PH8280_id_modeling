...
$$
\begin{aligned}
\frac{dS}{dt} &= \Lambda - \frac{\beta_1 S I_1}{N_0} - \frac{\beta_2 S I_2}{N_0} - \alpha S - \mu S \\
\frac{dI_1}{dt} &= \frac{\beta_1 S I_1}{N_0} - (\mu + \gamma_1) I_1 \\
\frac{dI_2}{dt} &= \frac{\beta_2 S I_2}{N_0} - (\mu + \gamma_2) I_2 \\
\frac{dR_1}{dt} &= \gamma_1 I_1 - \mu R_1 \\
\frac{dR_2}{dt} &= \gamma_2 I_2 - \mu R_2 \\
\frac{dV}{dt} &= \alpha S - \mu V
\end{aligned}
$$
...
The system is observed through three outputs, which incorporate the **reporting fraction** $\rho$ (`params10`):

$$
\begin{aligned}
y_1(t) &= \rho \cdot I_1(t) \quad \text{(Reported Cases, Strain 1)} \\
y_2(t) &= \rho \cdot I_2(t) \quad \text{(Reported Cases, Strain 2)} \\
y_3(t) &= V(t) \quad \hspace{1.1em} \text{(Observed Vaccinated Population)}
\end{aligned}
$$
